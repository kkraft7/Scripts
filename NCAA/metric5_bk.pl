...
#
# metric5.pl: Calculate a metric designed to estimate the level of
#             upsets for a given NCAA tournament.
#
# Change "$games" to "$games_played" or "$total_games"?
# One big mystery is why does $avg_diff_tot keep adding up to an odd integer?!
...
while (<DATA_FILE>) {
  ...
  if ( "$per_round" eq "TRUE" ) {
    print("\n  YEAR: $year\n");
    print("  ROUND   AVG DIFF   MIN DIFF   TOT DIFF   TOT SEED   NUM TEAMS\n");
    print("  =====   ========   ========   ========   ========   =========\n");
  }
...
print(  "=============   =============   =============\n");
for ($i = 0; $i < scalar(@key5b); $i++) {
  printf("%s: %7.2f   %s: %6.2f   %s: %6.2f\n",
