...
foreach $year (sort(keys(%seed_data))) {
  ...
  for ($i = 0; $i < $games; $i++) {
    ...
    $norm_wins = sprintf("%4.2f",  $loser/($winner + $loser));
    $norm_lose = sprintf("%4.2f", $winner/($winner + $loser));
        
    $norm_totals{$norm_wins}++;		# total games for winning seed
    $norm_totals{$norm_lose}++;		# total games for losing seed
    $norm_winner{$norm_wins}++;		# total wins for winning seed

    if ( "$DEBUG" eq TRUE ) {
      printf("INDEX: %2d; WINNER: %2d; LOSER: %2d\n", $i, $winner, $loser);
      print ("NORM_WINS = ${norm_wins}; NORM_LOSE = ${norm_lose}\n");
    }
  }
}
...
print("\nNORM FACT  WIN PERCENT  TOTAL GAMES\n");
print(  "---------  -----------  -----------\n");
foreach $fact (sort(keys(%norm_totals))) {

  # Note that $norm_totals{$fact} is always greater than 0
  printf("     %4.2f       %6.2f          %3d\n", $fact,
    (100*$norm_winner{$fact}/$norm_totals{$fact}), $norm_totals{$fact});
}
