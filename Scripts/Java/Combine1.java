...
*
* Usage: java Combine1 infile outfile [-gt] [-s Integer]
...
public class Combine1 {
    ...
    public static void main(String[] args) {
        ...
        if (args.length < 2) {
            System.out.println("Usage: java Combine1 infile outfile [-gt]");
            System.out.println("Option -gt is to indicate that '>' should be used.");
            ...
            for (int argNo = 2; argNo < args.length; argNo++) {
                ...
                if ( args[argNo].startsWith("-s")) {
                    ...
                    if ( args.length > argNo && ! args[argNo].startsWith("-") ) {
                        try {
                            maxSize = Integer.parseInt(args[argNo]);
                        } catch (NumberFormatException exception) {
                            System.out.println("Invalid size value '" + args[argNo] +
                                "' (must be an integer)");
                        }
                    }
                }
