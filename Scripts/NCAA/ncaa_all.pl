...
$rMAX            =  7;		# number of rounds for a 64-team tournament
...
#       $r, $max_diff_prob[$r], $r, $min_diff_prob[$r], $prob_mlt/$num_teams);
  ...
  # (1) Winning seed averaged over all games
  $avg_seed_tot{$y} = 100*($tot_seed     - $min_seed_tot)/(17*$games  -  2*$min_seed_tot);
  # (2) Differential between winning and losing seed averaged over all games
  $avg_seed_dif{$y} = 100*($tot_seed_dif - $min_seed_dif)/(-2*$min_seed_dif);
  # (3) Winning seed averaged per round, and over all rounds
  $avg_seed_per{$y} = ${tot_pcnt}/${rounds};
  # (4) Seed differentials averaged per round, and over all rounds
  $seed_dif_per{$y} = ${tot_diff_per}/${rounds};
  # (5) Winning seed normalization factor averaged over all games
...
@key3 = reverse(sort_key_by_value(%avg_seed_per));
@key4 = reverse(sort_key_by_value(%seed_dif_per));
@key5 = reverse(sort_key_by_value(%avg_norm_dif));
...
print("\nAVG PER GAME   AVG SEED DIF   AVG PER RND    SEED DIF PER   NRM SEED FCT" .
      "   PROB MLT DIF\n");
print(  "===========    ============   ============   ============   ============" .
      "   ============\n");

for ($i = 0; $i < scalar(@key1); $i++) {
  printf("%s: %6.2f   %s: %6.2f   %s: %6.2f   %s: %6.2f   %s: %6.2f   %s: %6.2f\n",
    $key1[$i], $avg_seed_tot{$key1[$i]}, $key2[$i], $avg_seed_dif{$key2[$i]},
    $key3[$i], $avg_seed_per{$key3[$i]}, $key4[$i], $seed_dif_per{$key4[$i]},
    $key5[$i], $avg_norm_dif{$key5[$i]}, $key6[$i], $dif_prob_mlt{$key6[$i]});
#   $key2[$i], $avg_seed_tot{$key2[$i]}, $key3[$i], $avg_seed_dif{$key3[$i]},
#   $key1[$i], $avg_seed_per{$key1[$i]}, $key5[$i], $seed_dif_per{$key5[$i]},
#   $key4[$i], $avg_norm_dif{$key4[$i]}, $key6[$i], $dif_prob_mlt{$key6[$i]});
}  
