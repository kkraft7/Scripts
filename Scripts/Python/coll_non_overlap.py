...
# 05) This uses telnetlib instead of sockets to talk to Crawl Manager
...
try:
  ...
  #     Note that the '\n' is required to send the command.
  print ('SETUP TELNET:\n%sPROC: CRAWL MANAGER\n%sHOST: %s\n%sPORT: %s' %
         (' ' * 8, ' ' * 8, crawlMgrHost, ' ' * 8, crawlMrgPort))
  telnetSession = telnetlib.Telnet(crawlMgrHost, crawlMrgPort)
  ...
  telnetSession.write(crawlMgrCmd + '\n')
  print 'SENT COMMAND: %s' % (crawlMgrCmd)
  response = telnetSession.read_until('ACKgoogle', 60)
...
except (telnetlib.socket.error, EOFError), exceptionData:
  print 'TELNET ERROR: FAILED TO CONNECT\n%sPROC: CRAWL MANAGER' % (' ' * 8)
  print '%sDATA %s\n' % (' ' * 8, str(exceptionData))
